//
//  Settings.h
//  HackerBooks
//
//  Created by Fernando on 3/31/15.
//  Copyright (c) 2015 f3rn4nd0. All rights reserved.
//

#pragma - KEYS USER DEFAULTS

#define IS_FIRST_RUN @"IS_FIRST_RUN"

#pragma - Settings

#define URL_JSON_DATA @"https://t.co/K9ziV0z3SJ"
#define NAME_LOCAL_FILE_DATA @"data.json"



#define NOTIFICATION_CHANGE_BOOK @"NOTIFICATION_CHANGE_BOOK"
#define NOTIFICATION_LOAD_BOOK @"NOTIFICATION_LOAD_BOOK"
#define FINISH_DOWNLOAD_BOOK @"FINISH_DOWNLOAD_BOOK"
#define FAVORITES_BOOKS @"FAVORITES_BOOKS"
#define LAST_BOOK @"LAST_BOOK"